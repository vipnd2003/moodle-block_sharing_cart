 ...
 *  
 *  @author  VERSION2, Inc.
 *  @version $Id: block_sharing_cart.js 798 2012-09-13 06:57:07Z malu $
 */
YUI.add('block_sharing_cart', function (Y)
{
	M.block_sharing_cart = new function ()
	{
		/** @var {Object}  The configurations */
		var conf = {
			icons: {
				// actions
				...
				'delete'  : { css: 'editing_delete' , pix: 't/delete'  },
			...
			str: {
				/* block_sharing_cart.php#get_hidden_strings() */
			}
		};
		...
		function show_error(response)
		{
			try {
				...
				new M.core.exception({
					name: conf.str['pluginname'] + " - " + conf.str['error'],
					message: ex.message,
					fileName: ex.file,
					lineNumber: ex.line,
		...
		{
			return $block.one('.editing_bulkdelete').get('href').match(/course=(\d+)/)[1];
		}
		 ...
		 *  
		 *  @param {String} name  The command name, predefined in conf.icons
		...
		function create_command(name, pix)
		{
			return Y.Node.create('<a href="javascript:void(0)"/>')
				.addClass(conf.icons[name].css)
				.set('title', conf.str[name])
				.append(
					Y.Node.create('<img class="iconsmall"/>')
						.set('alt', conf.str[name])
						.set('src', M.util.image_url(pix || conf.icons[name].pix))
					);
		}
			...
			this.show = function (id)
			{
				...
				function create_target(id, to)
				{
					var $anchor = Y.Node.create('<a href="javascript:void(0)"/>')
						...
						.set('title', conf.str['movehere'])
						.append(
							Y.Node.create('<img/>')
								.set('alt', conf.str['movehere'])
		...
		var restore_targets = new function ()
		{
			...
			this.show = function (id)
			{
				...
				$cancel.on('click', this.hide, this);
				$clipboard.append(conf.str['clipboard'] + ":").append($view).append($cancel);
				...
				$container.all(M.course.format.get_section_wrapper(Y)).each(function ($section)
				{
					...
					var $target = Y.Node.create('<a/>')
						.set('href', href)
						.set('title', conf.str['copyhere'])
						.append(
							Y.Node.create('<img/>')
								.set('alt', conf.str['copyhere'])
								.set('src', M.util.image_url('movehere'))
							);
		...
		var directories = new function ()
		{
			...
			{
				var pix = conf.icons[visible ? 'dir-open' : 'dir-closed'].pix;
		...
		{
			// get strings
			$block.all('form[name="strings"] input').each(function ()
			{
				conf.str[this.get('name')] = this.get('value');
			});
			conf.str['pluginname'] = $block.one('h2').get('text');
					...
					success: function (tid, response)
					{
						...
						if (copyable) {
							var $yesnocancel = new M.block_sharing_cart.yesnocancel({
								title: conf.str['backup'],
								question: conf.str['confirm_userdata'] + embed_cmid(cmid),
								yesLabel: conf.str['yes'], noLabel: conf.str['no'], cancelLabel: conf.str['cancel']
							});
							...
							//if (confirm(conf.str['confirm_backup']))
							//	backup(cmid, false);
							var $okcancel = new M.core.confirm({
								title: conf.str['backup'],
								question: conf.str['confirm_backup'] + embed_cmid(cmid),
								yesLabel: conf.str['ok'], noLabel: conf.str['cancel']
							});
		...
		{
			if (!confirm(conf.str['confirm_delete']))
				return;
